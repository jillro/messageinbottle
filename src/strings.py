STATUS = """

(You have {balloons} 🎈 left!)
"""

WELCOME = """
Welcome ! This is Helium bot.

Send a message to the bot. Someone else will get it, and you will get some previous user message.

Write to specific channels using hashtags in your message. You will receive a message with the same \
hashtags. A good start is to use your language (#es, #fr, #de, #ru...).

Visit https://heliumbot.site for more informations.
"""

NO_MESSAGE_EVER = """
Sorry! No message was ever sent with this hashtags combination... but yours will be sent if \
someone visit those hashtags after you.
"""

YOU_AGAIN = """
Sorry ! The last person to send a message here was...you ! So we just replaced the first message by the new one.  
"""

MESSAGE_INTRO = """
Thanks for your message! It will be delivered to the next person. Here is a message from {}:

"""

REPLY_INTRO = """
{} replied to your message!

"""

NO_MORE_balloon = """
No more balloons!
"""

BALLOONS_HELP = """
You just received your first message. To start a conversation, you can :
* click the dedicated button under the message
* use the reply feature of your messaging app

Write your reply with care: you can only reply once per message.

If you do not want to reply, but you think the person should be able to send more messages, give them a \
free balloon. Please help people who write long and interesting messages, which can lead to inspiring connections.

You have 24 hours to answer each message. Then, it is lost.
"""


TOO_SHORT = """
A good message is no less than 50 characters. Remember, this is the only thing the other person will know about you! \
To help you, hashtags do count in the 50 characters.

Please write your message again :
"""
